...
if (isset($_POST['method'])) {
	...
	else if($_POST['method'] === "upcard"){
		$currentorder = "";
		$id2 = "";
		$id= $_POST['id'];
		$sql = "SELECT reorder FROM $tableName WHERE id = $id";
		$result = mysqli_query($conn, $sql);
			if ($row = mysqli_fetch_assoc($result)){
				$currentorder = $row['reorder'];
		}
		//get id of the one to be switched
		$sqe = "SELECT id FROM $tableName WHERE reorder = $currentorder - 1";
		$result = mysqli_query($conn, $sqe);
			if ($row = mysqli_fetch_assoc($result)){
				$id2 = $row['id'];
		}

		$sqr = "UPDATE $tableName SET reorder =reorder -1 WHERE id = $id";
		mysqli_query($conn, $sqr);

		$sqt = "UPDATE $tableName SET reorder = reorder + 1 WHERE id = $id2";
		mysqli_query($conn, $sqt);

	}
	else if($_POST['method'] === "downcard"){
		$currentorder = "";
		$id2 = "";
		$id= $_POST['id'];
		$sql = "SELECT reorder FROM $tableName WHERE id = $id";
